...
function drawCardTexture(gl, image, title, duration) {
  ...
  // Centering triangle visually usually needs a slight offset to right
...
class Media {
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          // Simple alpha discard if transparent edges
    ...
    img.src = this.image;
    img.onload = () => {
      // Generate the card texture once image is loaded
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // Efficient Aspect Ratio Sizing
    // Card Aspect Ratio: 16:9
    const aspect = 16 / 9;

    // Card Height: 85% of grid height (viewport.height) to prevent clipping
    ...
    this.padding = cardWidth * 0.05; // 5% padding
  ...
  createMedias(items, bend = 1) {
    ...
    this.medias = this.mediasImages.map((data, index) => {
      return new Media({
        ...
        screen: this.screen,
        text: data.title,
        duration: data.duration, // Pass duration
...
export default function CircularGallery({
  ...
  return (
    <div
      className="w-full h-52 overflow-hidden cursor-grab active:cursor-grabbing"
      ref={containerRef}
    />
  );
}
